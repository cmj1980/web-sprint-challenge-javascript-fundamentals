...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Because internal is local scoped inside of the myFunction and nestedFunction is also inside of myFunction. 
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooArray){
    let displayNames = []
    zooArray.forEach(function(value) {
  ...
  */
  ...
  function lowerCaseNames(zooArray){
    return zooArray.map(item => item.animal_name.toLowerCase());
  }
  ...
  */
  ...
  function lowPopulationAnimals(zooArray){
    return zooArray.filter(item => item.population < 5);
  }
  ...
  */
  ...
  function USApop(zooArray){
    return zooArray.reduce((acc, item) => acc + item.population, 0);
   }
